...
namespace HUX.Interaction
{
    ...
    public class BoundingBoxManipulate : BoundingBox
    {
        ...
        protected override void OnManipulationStarted(GameObject obj, InteractionManager.InteractionEventArgs eventArgs)
        {
            ...
            HoloDebug.Log("OnManipulationStarted");
        ...
        {
            HoloDebug.Log("OnManipulationCanceled");
            ManipulatingNow = false;
        ...
        {
            HoloDebug.Log("OnManipulationCompleted");
            ManipulatingNow = false;
        ...
        protected override void Update()
        {
            ...
            {
                if(ManipulatingNow==true)
                HoloDebug.Log("invisible");
                ManipulatingNow = false;
            }*/
            ...
            if (ManipulatingNow)
            {
                ...
                switch (CurrentOperation)
                {
                    ...
                    case OperationEnum.ScaleZ:
                        ...
                        Vector3 anchorDirection = (activeHandle.transform.position - targetBoundsWorldCenter).normalized;
                        Vector3 dragDirection = -smoothVelocity.normalized;
                        float boolScale = Vector3.Dot(anchorDirection, dragDirection);

                        float dragAmount = orientedVelocity.x;
                        if (boolScale > 0)
                        {
                            dragAmount = Mathf.Abs(dragAmount);
                        }
                        else
                        {
                            dragAmount = -Mathf.Abs(dragAmount);
                        ...
                        //HoloDebug.Log("orientedVelocity :" + orientedVelocity.x + " ," + orientedVelocity.y + " ," + orientedVelocity.z + " ," + " handleScreenPoint:" + handleScreenPoint + " gizmoScreenPoint:" + gizmoScreenPoint + "activeHandle:" + activeHandle);
                        //HoloDebug.Log(new StringBuilder().AppendLine().ToString());
        ...
        {
            //HoloDebug.Log("accept:"+acceptInput.ToString());
            //HoloDebug.Log("manipulating:"+ManipulatingNow.ToString());
            // Goes without saying
            if (!acceptInput)
                return;
        ...
        {
            HoloDebug.Log("TryToSetHandle");
            if (!acceptInput)
            {
                HoloDebug.Log("Not accepting input");
                return;
            }
