...
namespace HoloGeek {
    public class ImportExportAnchorManager : GDGeek.Singleton<ImportExportAnchorManager> {
        ...
        void Start()
        {
            currentState = ImportExportState.Start;
        ...
        private void _Start()
        {
            ...
            //currentState = ImportExportState.Start;
            HoloDebug.Log("！" + HoloHelper.HasServer());
            if (HoloHelper.IsEditor()||!HoloHelper.HasServer())
            {
        ...
        void InitRoomApi()
        {
            ...
            if (roomManager.GetRoomCount() == 0)
            {
                ...
                if (HoloHelper.IsRoomCreater())
                {
                    HoloDebug.Log("Creating room ");
        ...
        public ImportExportState oldState = ImportExportState.Start;
        void Update()
        {
            if (oldState != currentState)
            {
                HoloDebug.Log("now state:" + currentState.ToString());
