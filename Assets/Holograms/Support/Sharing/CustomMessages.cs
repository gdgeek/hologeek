...
public class CustomMessages : SpectatorView.SV_Singleton<CustomMessages>
{
    ...
    {
        HeadTransform = MessageID.UserMessageIDStart,//头位置
        UserAvatar,//
        UserHit,//用户打击
        ShootProjectile,//射击
                        //  HatTranform,
    ...
    /*
    public void SendHatTransform(Vector3 position, Quaternion rotation, byte HasAnchor) {
        // If we are connected to a session, broadcast our head info
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.HatTranform);
    ...
    public void SendInfo(int maPos, int songPos, int score, int time){
        // If we are connected to a session, broadcast that the target exploded.
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send.
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.Info);
            msg.Write(maPos);
            msg.Write(songPos);
            msg.Write(score);
            msg.Write(time);
